        ...
        try {
            ...
            if (reasoningButtons.length > 0) {
                ...
                for (let i = 0; i < reasoningButtons.length; i++) {
                    ...
                    // Try multiple strategies to find reasoning content
                    let reasoningFound = false;

                    // Strategy 1: Look for expanded content near button
                    if (parentContainer) {
                        // Search in siblings after the button
                        let siblings = Array.from(parentContainer.parentElement.children);
                        let buttonIndex = siblings.indexOf(parentContainer);

                        for (let j = buttonIndex + 1; j < siblings.length && j < buttonIndex + 10; j++) {
                    ...
                    if (!reasoningFound) {
                        // Try finding reasoning in nearby elements
                        let nearbyElements = d.querySelectorAll('[class*="reasoning"], [class*="step"], [data-reasoning]');
                        nearbyElements.forEach(el => {
                            if (el.offsetParent !== null && el.innerText.length > 50 && el.innerText.length < 5000) {
                                o += el.innerText.trim() + "\n";
                                reasoningFound = true;
                            }
                        });
                    ...
                    // Strategy 3: Use text extraction from button's neighborhood
                    if (!reasoningFound) {
                        let grandParent = btn.parentElement?.parentElement;
                        if (grandParent) {
                            let allText = grandParent.innerText;
                            ...
                            if (stepsIndex >= 0) {
                                ...
                                let lines = afterSteps.split("\n")
                                    .filter(l => l.trim().length > 5)
                                    .slice(0, 20);

                                if (lines.length > 0) {
                                    lines.forEach(line => {
                                        if (!line.includes("sources") && !line.includes("citations")) {
                                            o += line.trim() + "\n";
                                        }
                                    });
                                    reasoningFound = true;
                    ...
                    if (!reasoningFound) {
                        o += "> Unable to extract reasoning content\n";
                    }

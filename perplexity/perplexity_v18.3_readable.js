        ...
        try {
            ...
            if (reasoningButtons.length > 0) {
                ...
                for (let i = 0; i < reasoningButtons.length; i++) {
                    ...
                    // Find reasoning content - look for markdown container
                    let reasoningFound = false;

                    // Strategy: Find the closest markdown-content div after the button
                    let buttonParent = btn.closest('[id^="radix"]');
                    if (buttonParent) {
                        // Look for div with markdown content
                        let markdownDivs = buttonParent.querySelectorAll('[id^="markdown-content"]');
                        if (markdownDivs.length > 0) {
                            // Use the last one (most likely to be the reasoning content)
                            let contentDiv = markdownDivs[markdownDivs.length - 1];
                            let reasoningContent = md(contentDiv);

                            if (reasoningContent.trim().length > 50) {
                    ...
                    // Fallback: Look for any div with prose/markdown classes near button
                    if (!reasoningFound) {
                        let parent = btn.parentElement;
                        while (parent && !parent.classList.contains('prose') && parent !== d.body) {
                            parent = parent.parentElement;
                        }

                        if (parent && parent.classList.contains('prose')) {
                            o += md(parent).trim() + "\n\n";
                            reasoningFound = true;
                        }
                    }
